...
>+ 7.22 第二次组会，重新熟悉流程，确定每个人的分工，完成代码熟悉、接口规范和功能规划
>+ 7.29 第三次组会，在助教帮助下对每个人实现代码进行了优化
        采用B树存储而不是hash表，采用定时刷新检查点的方式recovery
>+ 8.05 基本各自完成第一阶段的代码
***
...
### 指导老师：毛嘉莉
### 项目简介：
> 以出租车GPS数据为基础，通过海量轨迹数据挖掘识别道路交叉路口，与企业对接（滴滴公司），利用实时轨迹点更新和细化矢量地图。
...
>+ 8.18 在线与老师、研究生交流，汇报最新进展，用四叉树的方法完成全区交叉路口位置识别工作。
>#### Intersection structure
>
>+ 9.4 使用MeanShift与DBSCAN聚类相结合的方式，找到交叉路口中心点的区域，本算法在大范围轨迹区域上有明显优势。
>+ 9.13 尝试得到交叉路口的大小，效果不理想。
***
...
>+ 7.20 开始分配任务，每位同学每天开始找新闻（每个公司正例10篇，负例10篇），这些新闻将作为日后的语料库。
>+ 7.20 同时对本科生提出学习要求，学习word2vec原理  和  NLP的相关算法。
>+ 7.26 部分同学，开始着手爬虫，运用爬虫在特定网站得到相关新闻。
>+ 9.15 开始进行下一阶段，要求熟悉NER 和 机器学习事件分类算法。同时要求大家开始学习EM算法，和SVM算法，要求熟悉原理，便于日后做算法设计。
...
>+ 8.18 我们提出了CNN-MDN架构，实验中神经网络可以学会看图生成笔画序列。
...
>+ 8.1  组会 确认分工 在知网扩充数据量进行关联分析，做出随年份的思想动态变化，在抖音上集中在关键词上对视频描述和评论进行爬取，再进行分析尝试
***
...
>+ 8.13 图结构模块化，可继承后分别实现最短路径、连通分量。通过不同的分组方式，包括随机、哈希等，并将其分成不同的组数，模拟每一种情况的信息丢失，并将其增加的迭代次数与原迭代次数进行比较，初步做图可视化。
...
### 指导老师：毛嘉莉
### 项目简介：
> 理解新路发现，熟悉论文和实验代码，做一个demo

### 目前进度：
>+ 7.16 确定任务，开始学习论文和实验代码
>+ 8.1  实现代码复现
***
